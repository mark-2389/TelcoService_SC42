...
def generate_insert_command(table_name, list_of_values, keys=[]):
    ...
    keys: [str]
        The list of the attributes' names. If the list is empty then it's
...
def generate_insert(table_name, generator, number_of_iterations, keys=[]):
    ...
    for i in range(number_of_iterations):
        ...
        # if the value returned by the generator is null we want to return
        if val == None:
...
def sanitise_filename(filename):
    """
    Generate a new filename if the filename passed as argument already exists.
